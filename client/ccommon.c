...
int get_stu_msg(STU_M *stu) {
    ...
    while (n < 0) {
        color_print(line + 1, right, COLOR_RED, COLOR_BLACK, "(输入错误，请重新输入 学生数不能小于0)", &color_flag);
        move(line, 25);                             //移动
        addstr(menu_str[0]);
        move(line, 25);                             //移动
        refresh();                                  //刷新
    ...
    ++line;
    sprintf(in, "学号");                              //格式化输出到in
    move(line, right);
    addstr(in);
    right += 10;
    sprintf(in, "姓名");                              //格式化输出到in
    move(line, right);
    addstr(in);
    for (int i = 0; i < stu->course_num; i++) {
        right += 10;
        sprintf(in, "%s", stu->course_name[i]); //循环格式化输出到in
        move(line, right);
        addstr(in);
    }
